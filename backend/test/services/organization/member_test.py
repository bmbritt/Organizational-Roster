...
from ..fixtures import member_service_integration, user_svc_integration, event_svc_integration
...
def test_delete_self(member_service_integration: MemberService):
    ...
    fetched_roster = member_service_integration.all(1)
    member_service_integration.deleteSelf(User(id=1, pid=1,first_name="",last_name="",pronouns="",github="",onyen="",email="",github_id=1,github_avatar="1",accepted_community_agreement=True), cads)
...
def test_add_member_public(member_service_integration: MemberService):
    ...
    fetched_roster = member_service_integration.all(1)
    member_service_integration.add_member_public(User(id=5, pid=5,first_name="Evan",last_name="Menendez",pronouns="",github="",onyen="",email="",github_id=4,github_avatar="4",accepted_community_agreement=True), cads)
